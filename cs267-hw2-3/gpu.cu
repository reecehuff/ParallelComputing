...
__global__ void compute_forces_gpu(particle_t* particles, int num_parts, double bin_width, int num_bins_across, int* particle_ids, int* bin_ids) {
    ...
    // Zero out the particle's acceleration
    ...
    // Loop through neighboring bins and apply force to particle tid
    for(int x = max(bin_x-1, 0); x < min(bin_x+2,num_bins_across); ++x){
        for(int y = max(bin_y-1, 0); y < min(bin_y+2,num_bins_across); ++y){
            for(int k = bin_ids[x+num_bins_across*y]; k < bin_ids[x+num_bins_across*y+1]; ++k)  {
                int part = particle_ids[k];
                apply_force_gpu(particles[tid], particles[part]);
            }
        }
    }
}
...
void simulate_one_step(particle_t* parts, int num_parts, double size) {
    ...
    // Zero out the arrays
    cudaMemset(bin_ids     , 0, (num_bins+1) * sizeof(int));
    ...
    // before prefix sum, bin_ids = {4  2  1  5  3, 0}
    // after  prefix sum, bin_ids = {0  4  6  7 12, 15}
    thrust::exclusive_scan(thrust::device, bin_ids, bin_ids+num_bins+1, bin_ids); 
    ...
    // Compute forces
    compute_forces_gpu<<<num_blocks, NUM_THREADS>>>(parts, num_parts, bin_width, num_bins_across, particle_ids, bin_ids);
